--| Helper v1.1 for no rewrite on every file exension
--|
--|=== Change Log :
--| ---------------
--| * v1.0 - 18 Juli 2021
...
function GetReferer(s)
	s = string.find(s, [[Referer: *(\S+)\r?\n]])
	if s==nil then return -1 else return string.lower(s) end
end

function GetCookie(s)
	_,_,x = string.find(s, '[cC]ookie:.*\r?\n')
	if s==nil then return -1 else return string.lower(s) end
end
